...
export function Layout({ user, onLogout }: { user: AuthUser; onLogout: () => void }) {
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-border flex justify-around py-2 z-50">
        {links.map(l => (
          <NavLink
            key={l.to}
            to={l.to}
            ...
            className={({ isActive }) =>
              cn(
                'flex flex-col items-center gap-0.5 text-[11px] py-1 px-3 rounded-xl transition-all duration-200',
